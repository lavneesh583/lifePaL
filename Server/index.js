...
app.post("/createUsers", (req, res) => {
  const query = {
    text: "CREATE TABLE users(user_id VARCHAR PRIMARY KEY,name VARCHAR(20), email VARCHAR(30), password VARCHAR(255) NOT NULL, address VARCHAR(100), units INT , exigency boolean DEFAULT false,condition_certi bytea, ph_no INT , adhar_no INT, blood_grp VARCHAR(38));",
  };
...
app.post("/insertIntoBloodBank", (req, res) => {
  let { bank_id, b_name, b_address, blood_grp, available_units, quotation } =
    req.body;
...
app.post(
  "/insertRequest",
  condition_certificate.array("condition_certificate", 3),
  (req, res) => {
    const imagesArray = [];
    let {
      name,
